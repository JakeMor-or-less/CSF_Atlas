...
```
```{r}
# Group the sample by age
metadata$age_group <- ifelse(metadata$age <= "25", "young", "old")
```
...
```{r}
# Get sample names that are non-senescent
non_sen_samples <- rownames(colData(dds)[colData(dds)$putative_sen == "non_senescent", ])

# Subset the DESeq2 object
dds_non_sen <- dds[, non_sen_samples]

# Get sample names that are senescent
sen_samples <- rownames(colData(dds)[colData(dds)$putative_sen == "senescent", ])

# Subset the DESeq2 object
dds_sen <- dds[, sen_samples]
```

```{r}
# Check your current design
design(dds_non_sen) <- ~ 1
design(dds_non_sen)
...
vsd_non_sen <- vst(dds_non_sen, blind = FALSE)

# Get normalized counts for heatmap
vsd_sen <- vst(dds_sen, blind = FALSE)
```
...
annotation_colors <- list(
  Senescence = c("senescent" = "red", "non_senescent" = "blue"),  # Adjust as needed
  Age_Group = c("young" = "green", "old" = "orange"),
  Sex = c("M" = "yellow", "F" = "purple")
)
...
```{r}
pcaData <- plotPCA(vsd, intgroup=c("putative_sen", "age_group"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca1 <- ggplot(pcaData, aes(PC1, PC2, color = putative_sen, shape = age_group)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples",
       subtitle = "ntop=500 features by variance",
  ...
  theme(
    ...
    panel.grid.major = element_line(
      color = "gray80",      # Darker gray
      size = 0.3,           # Thicker lines
      linetype = "dashed"    # Solid lines (default)
    ),
    panel.grid.minor = element_line(
      color = "gray90",      # Light gray
      size = 0.3,           # Thin lines
      linetype = "dashed"   # Dotted lines
    ))
ggsave("PCA_plot_sen_vs_age.png", plot = pca1, width = 8, height = 6, dpi = 300)
```
```{r}
pcaData <- plotPCA(vsd, intgroup=c("putative_sen", "sex"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca2 <- ggplot(pcaData, aes(PC1, PC2, color = putative_sen, shape = sex)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples",
...
ggsave("PCA_plot_sen_vs_sex.png", plot = pca2, width = 8, height = 6, dpi = 300)
```


```{r}
pcaData <- plotPCA(vsd, intgroup=c("dataset", "age_group"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca3 <- ggplot(pcaData, aes(PC1, PC2, color = dataset, shape = age_group)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples",
...
ggsave("PCA_plot_dataset_vs_age.png", plot = pca3, width = 8, height = 6, dpi = 300)
```
```{r}
colnames(colData(vsd_non_sen))
colData(vsd_non_sen)
```


```{r}
pcaData <- plotPCA(vsd_non_sen, intgroup=c("sex", "age_group"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca10 <- ggplot(pcaData, aes(PC1, PC2, color = sex, shape = age_group)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples (Non-Senescent)",
       subtitle = "ntop=500 features by variance",
       color = "Sex",
       shape = "Age Group") +
  theme_classic() +
...
print(pca10)
ggsave("PCA_plot_nonsen_sex_vs_age.png", plot = pca10, width = 8, height = 6, dpi = 300)
```


```{r}
pcaData <- plotPCA(vsd_sen, intgroup=c("sex", "age_group"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca11 <- ggplot(pcaData, aes(PC1, PC2, color = sex, shape = age_group)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples (Senescent)",
       subtitle = "ntop=500 features by variance",
       color = "Sex",
       shape = "Age Group") +
  theme_classic() +
...
pca11
ggsave("PCA_plot_sen_sex_vs_age.png", plot = pca11, width = 8, height = 6, dpi = 300)
```


```{r}
pcaData <- plotPCA(vsd_non_sen, intgroup=c("age_group", "dataset"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca20 <- ggplot(pcaData, aes(PC1, PC2, color = age_group, shape = dataset)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples (Non-Senescent)",
...
pca20
ggsave("PCA_plot_nonsen_age_vs_dataset.png", plot = pca20, width = 8, height = 6, dpi = 300)
```


```{r}
pcaData <- plotPCA(vsd_sen, intgroup=c("age_group", "dataset"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca21 <- ggplot(pcaData, aes(PC1, PC2, color = age_group, shape = dataset)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples (Senescent)",
...
pca21
ggsave("PCA_plot_sen_age_vs_dataset.png", plot = pca21, width = 8, height = 6, dpi = 300)
```


```{r}
pcaData <- plotPCA(vsd_non_sen, intgroup=c("age_group", "disease"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca30 <- ggplot(pcaData, aes(PC1, PC2, color = age_group, shape = disease)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples (Non-Senescent)",
...
pca30
ggsave("PCA_plot_nonsen_age_vs_disease.png", plot = pca30, width = 8, height = 6, dpi = 300)
```


```{r}
pcaData <- plotPCA(vsd_sen, intgroup=c("age_group", "disease"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
# Create custom ggplot
pca31 <- ggplot(pcaData, aes(PC1, PC2, color = age_group, shape = disease)) +
  geom_point(size = 5) +
  xlab(paste0("PC1: ", percentVar[1], "% variance")) +
  ylab(paste0("PC2: ", percentVar[2], "% variance")) +
  labs(title = "PCA of CD8 T Cell Samples (Senescent)",
...
pca31
ggsave("PCA_plot_sen_age_vs_disease.png", plot = pca31, width = 8, height = 6, dpi = 300)
```
